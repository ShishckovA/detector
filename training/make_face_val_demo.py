...
def summarize(predictions: list[dict[str, Any]], threshold: float) -> dict[str, Any]:
    ...
    counts = Counter((int(row["label_id"]), int(row["pred_label_id"])) for row in predictions)
    tn = counts[(0, 0)]
    fp = counts[(0, 1)]
    fn = counts[(1, 0)]
    tp = counts[(1, 1)]
...
def row_card(row: dict[str, Any]) -> str:
    ...
    confidence = prob if pred_label == "positive" else 1.0 - prob
      ...
      <article class="card {status}" data-status="{status}" data-outcome="{outcome}" data-true="{html.escape(true_label)}" data-pred="{html.escape(pred_label)}" data-group="{group}" data-path="{title}">
        ...
        <div class="card-body">
          <div class="row">
            ...
            <span class="prob">p+ {prob:.3f}</span>
          </div>
          <div class="labels">
...
def write_html(path: Path, predictions: list[dict[str, Any]], summary: dict[str, Any], checkpoint_path: Path) -> None:
    sorted_rows = sorted(
        ...
        key=lambda row: (
            bool(row["correct"]),
            abs(float(row["prob_positive"]) - 0.5),
        ),
    )
    false_positives = [row for row in sorted_rows if row["label_id"] == "0" and row["pred_label_id"] == 1]
    false_negatives = [row for row in sorted_rows if row["label_id"] == "1" and row["pred_label_id"] == 0]
    uncertain = sorted(predictions, key=lambda row: abs(float(row["prob_positive"]) - 0.5))[:48]
    confident_correct = sorted(
        [row for row in predictions if row["correct"]],
        key=lambda row: abs(float(row["prob_positive"]) - 0.5),
        reverse=True,
    )[:48]
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .confusion {{ display: grid; grid-template-columns: repeat(2, minmax(120px, 1fr)); max-width: 420px; border: 1px solid var(--line); background: var(--panel); }}
    .cell {{ padding: 12px; border: 1px solid var(--line); }}
...
<body>
  <header>
    ...
    <div class="sub">Threshold: {summary["threshold"]:.2f}; validation rows: {summary["rows"]}; labels: {html.escape(json.dumps(summary["labels"], ensure_ascii=False))}</div>
    <div class="metrics">
      ...
      <div class="metric"><strong>{fmt_float(summary["roc_auc"])}</strong><span>ROC-AUC</span></div>
  ...
  <main>
    <section>
      ...
      <div class="confusion">
        <div class="cell"><b>{confusion["tn"]}</b>true negative</div>
        <div class="cell"><b>{confusion["fp"]}</b>false positive</div>
        <div class="cell"><b>{confusion["fn"]}</b>false negative</div>
        <div class="cell"><b>{confusion["tp"]}</b>true positive</div>
      </div>
    </section>
    <section>
      ...
      <div class="controls">
        ...
        <button data-filter="tp">TP ({confusion["tp"]})</button>
        <button data-filter="tn">TN ({confusion["tn"]})</button>
        <button data-filter="fp">FP ({confusion["fp"]})</button>
        <button data-filter="fn">FN ({confusion["fn"]})</button>
        ...
        <button data-filter="positive">True label positive ({summary["labels"].get("positive", 0)})</button>
        <button data-filter="negative">True label negative ({summary["labels"].get("negative", 0)})</button>
    ...
    <div id="derived-sections">
      {render_section("False Positives", false_positives[:48], "No false positives.")}
      {render_section("False Negatives", false_negatives[:48], "No false negatives.")}
      {render_section("Most Uncertain", uncertain, "No examples.")}
